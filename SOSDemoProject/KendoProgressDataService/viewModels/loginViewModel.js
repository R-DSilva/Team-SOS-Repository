...
(function(parent) {
    var loginViewModel = kendo.observable({
        ...
        login: function (E) {
            var list = [{ user: 'ricky', password: 'ricky', usertypeid: 1, name: "Ricky" },
                    { user: 'sam', password: 'sam', usertypeid: 1, name: "Sameer" },
                    { user: 'lipsa', password: 'lipsa', usertypeid: 1, name: "Lipsa" },
                    { user: 'mark', password: 'mark', usertypeid: 1, name: "Mark" },
                    { user: 'shams', password: 'shams', usertypeid: 1, name: "Shams" },
                    { user: 'duncan', password: 'duncan', usertypeid: 1, name: "Duncan" }]
            ...
            while (i < length) {
                if (list[i].user == this.get("username").toLowerCase() && list[i].password == this.get("password").toLowerCase()) {
                    window.location.href = "views/Dashboard.html?userName=" + list[i].name;
                    success = true;
                }
                i++;
            }
        ...
        //    if (this.get("username") == "lipsa" && this.get("password") == "test") {
